...
const DecorationBookDetail = () => {
  ...
  const { booking, decorationBooking, bookedAmenities, customer, decorationService} = decorationData;

  return (
    <div className="container mt-5">
      <div className="card shadow-lg border-0" style={{ backgroundColor: "#f9f9f9" }}>
        <div className="card-header bg-dark text-white text-center">
          <h2>{decorationService.name}</h2>
        </div>
        <div className="card-body">
          ...
          <div className="row mb-3">
            <div className="col-md-6">
              <strong>Manager name:</strong> {decorationService.managerName}
            </div>
            <div className="col-md-6">
              <strong>Manager name:</strong> {decorationService.managerNumber}
            </div>
          </div>
          <div className="row mb-3">
            <div className="col-md-6">
            ...
            <div className="col-md-6">
              <strong>Decoration Type:</strong> {decorationBooking.decoratedService}
            </div>
          </div>
          <hr />
          <h5 className="card-title text-secondary mb-4">Amenities</h5>
          <div className="row mb-3">
            {bookedAmenities.map((amenity) => (
              <div className="col-md-6" key={amenity.id}>
                <strong>{amenity.amenity}:</strong> {amenity.count} item(s) -{" "}
                {amenity.cost} PKR
              </div>
            ))}
          </div>
...
export default DecorationBookDetail;
